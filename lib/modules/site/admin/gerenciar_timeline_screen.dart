...
class _GerenciarTimelineScreenState extends State<GerenciarTimelineScreen> {
  ...
  void _mostrarSnackBar(String mensagem, {bool isErro = false}) {
    final bg = isErro ? context.uai.error : context.uai.success;
  ...
  Widget _buildFormHero(bool isEditando) {
    final t = context.uai;
    final cor = _ensureVisible(_getCorPorTipo(_tipoSelecionado), t.background);
    final onCor = _readableOn(cor);

    return Container(
      padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [cor, Color.alphaBlend(onCor.withOpacity(0.16), cor)],
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final icon = Container(
            ...
            height: 62,
            decoration: BoxDecoration(
              color: onCor.withOpacity(0.14),
              borderRadius: BorderRadius.circular(22),
              border: Border.all(color: onCor.withOpacity(0.16)),
            ),
            child: Icon(
  ...
  Widget _buildImagesBlock({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: Color.alphaBlend(accent.withOpacity(0.06), t.card),
  ...
  Widget _whiteSwitchChip() {
    final onPrimary = _onPrimary();
    final active = context.uai.success;
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _exibirPortfolioNoSite
              ? active.withOpacity(0.28)
              : onPrimary.withOpacity(0.14),
          borderRadius: BorderRadius.circular(99),
          border: Border.all(color: onPrimary.withOpacity(0.16)),
        ),
        child: Row(
  ...
  Widget _buildCardPublicacaoLista(Map<String, dynamic> data, String id) {
    ...
    final corTipo = _ensureVisible(_getCorPorTipo(tipo), t.card);
    ...
    return InkWell(
      ...
      child: Container(
        decoration: _cardDecoration(borderColor: corTipo.withOpacity(0.16)),
        child: ClipRRect(
          borderRadius: BorderRadius.circular(t.cardRadius),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
  ...
  Widget _buildImagemFallback(String tipo, Color corTipo) {
    final t = context.uai;
    final accent = _ensureVisible(corTipo, t.card);

    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            Color.alphaBlend(accent.withOpacity(0.16), t.cardAlt),
            Color.alphaBlend(accent.withOpacity(0.05), t.cardAlt),
          ],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setStateDialog) {
            ...
            final primaryFg = _readableOn(primary);
      ...
      child: SingleChildScrollView(
        ...
        child: Container(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.timeline_rounded, size: 74, color: t.textMuted),
              const SizedBox(height: 14),
              Text(
