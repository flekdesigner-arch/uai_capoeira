...
class _EditarVendaScreenState extends State<EditarVendaScreen> {
  ...
  void initState() {
    ...
    _observacoesController = TextEditingController(
      text: widget.vendaData['observacoes'] ?? '',
    );
  ...
  Future<void> _selecionarItem() async {
    ...
    if (itemSelecionado != null && mounted) {
      setState(() {
        final novoItem = {...itemSelecionado, 'quantidade': 1};
        _itens.add(novoItem);
                      ...
                      children: [
                        _buildInfoRow(
                          ...
                          DateFormat('dd/MM/yyyy HH:mm').format(
                            (widget.vendaData['data_venda'] as Timestamp?)
                                    ?.toDate() ??
                                DateTime.now(),
                          ),
                        ),
  ...
  Widget _buildInfoRow(
    ...
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label, style: TextStyle(color: context.uai.textSecondary)),
        Text(
          value,
          style: TextStyle(
